...
struct pt_regs_t {
		...
		uint32_t edi;			// 从 edi 到eax 由 pusha 指令压入
		uint32_t esi;
		uint32_t ebp;
		uint32_t esp;
		uint32_t ebx;
...
void isr16();					// 16 #MF 浮点处单元错误
void isr17();					// 17 #AC 对齐检查
void isr18();					// 18 #MC 机械检查
...
#define IRQ8	40	//	及时时钟
#define IRQ9	41	//	与 ORQ2 相接，可以设定给其他硬件
